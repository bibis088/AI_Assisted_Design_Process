...
## Règle logging — obligatoire

Toujours passer `skillNames: "figma-generate-design"` dans chaque appel `use_figma` de ce skill.
C'est un paramètre de logging — il n'affecte pas l'exécution.

```javascript
// Exemple d'appel use_figma avec skillNames
// skillNames: "figma-generate-design"
```

---

## Override du texte des instances — setProperties()

Les instances de composants arrivent avec du texte placeholder ("Title", "Label", "Button").
**Utiliser `setProperties()` plutôt que `node.characters`** — c'est plus fiable car ça passe par les propriétés déclarées du composant.

```javascript
// Découvrir les propriétés TEXT d'un composant
const tempInstance = primaryButton.createInstance();
console.log(JSON.stringify(tempInstance.componentProperties));
// → { "Label#2:0": { type: "TEXT", value: "Button" }, "Has Icon#4:64": { type: "BOOLEAN" } }
tempInstance.remove();

// Override via setProperties()
btnInstance.setProperties({ "Label#2:0": "Se connecter" });

// Pour instances imbriquées — appeler setProperties() sur l'instance enfant
const nestedHeading = cardInstance.findOne(n => n.type === "INSTANCE" && n.name === "Text Heading");
if (nestedHeading) {
  nestedHeading.setProperties({ "Text#2104:5": "Mon titre réel" });
}

// Fallback direct sur node.characters — seulement si le texte n'est PAS géré par une propriété
```

---

## Import et application des styles (text + effect)

```javascript
// Découvrir les styles depuis les écrans existants
const frame = figma.currentPage.findOne(n => n.name.includes("US_"));
const styles = { text: new Map(), effect: new Map() };
frame.findAll(() => true).forEach(node => {
  if ('textStyleId' in node && node.textStyleId) {
...
const headingStyle = await figma.importStyleByKeyAsync("HEADING_STYLE_KEY");
textNode.textStyleId = headingStyle.id;
```

---

## search_design_system — stratégie complète

```javascript
// Composants — utiliser includeComponents: true
...
// Variables — utiliser includeVariables: true
// search_design_system({ query: "background", includeVariables: true })
// search_design_system({ query: "space", includeVariables: true })

// Styles — utiliser includeStyles: true
// search_design_system({ query: "heading", includeStyles: true })
// search_design_system({ query: "shadow", includeStyles: true })
```

**Toujours préférer l'inspection des écrans existants** — plus rapide et plus fiable que search_design_system.
Ne lancer search_design_system que si le fichier n'a pas encore d'écrans.

---

## Récupération d'erreurs (Error Recovery)

Ce skill travaille section par section — les erreurs sont naturellement isolées.

1. **STOP** — ne pas retenter immédiatement
2. **Lire l'erreur** avec attention
3. Si peu clair → `get_metadata` ou `get_screenshot` pour inspecter l'état du fichier
4. **Corriger le script** puis retenter
5. Les sections précédentes (appels réussis) restent intactes

---

## Bonnes pratiques (Best Practices)

- **Toujours inspecter avant de construire** — le DS a probablement déjà le composant, token ou style nécessaire
- **Chercher avec des synonymes** — "NavigationPill" peut être trouvé sous "pill", "nav", "tab" ou "chip"
- **Préférer les tokens DS aux valeurs en dur** — bindings pour couleurs, spacing, radius · styles pour typo et ombres
- **Préférer les instances aux constructions manuelles** — elles restent liées et se mettent à jour avec le DS
- **1 section par appel `use_figma`** — jamais plus
- **Retourner les node IDs de chaque appel** — nécessaires pour les appels suivants et la récupération d'erreur
- **Valider visuellement après chaque section** — `get_screenshot` par nodeId (pas uniquement la page complète)
- **Respecter les conventions existantes** — si des écrans existent, matcher leur nommage et sizing
...
- `variable-patterns.md` → create/bind variables · import library variables · scopes · aliasing
- `text-style-patterns.md` → create/apply text styles · type ramps
- `effect-style-patterns.md` → shadows · blurs · import library styles
- `gotchas.md` → HUG/FILL interactions · counterAxisAlignItems · sizing order · paint issues
